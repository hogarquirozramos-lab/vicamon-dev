...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    ...
    try {
      ...
      if (msg.type === 'redeem_physical_code') {
        ...
        activePhysicalCodes.set(code, p.wallet); 
      ...
      if (msg.type === 'surrender') { const b = battles.get(msg.battleId); if (!b) return; if (b.isTeamBattle) { const otherId = b.p1id === id ? b.p2id : b.p1id; const winnerTeam = b.p1id === otherId ? b.team1 : b.team2; const winnerRemainingHp = winnerTeam.reduce((sum, st) => sum + Math.max(0, st.hp), 0); await endTeamBattle(msg.battleId, otherId, id, winnerRemainingHp); } else if (b.isGauntlet) await endGauntlet(msg.battleId, id, false); else if (b.isCpu) await endBattle(msg.battleId, CPU_ID, id, 0, true); else if (b.p1id === id || b.p2id === id) { const otherId = b.p1id === id ? b.p2id : b.p1id; await endBattle(msg.battleId, otherId, id, 0, true); } }

      if (msg.type === 'challenge_gauntlet') { const pl = lobby.get(id); if (!pl || pl.inBattle) return; if (pl.isGuest) { send(ws, { type: 'error', msg: 'Los invitados no pueden entrar a la Torre.' }); return; } if (msg.beast) pl.beast = msg.beast; if (!await hasHP(pl.wallet, 100)) { send(ws, { type: 'error', msg: 'Necesitas 100 HP para la Torre.' }); return; } await lockHP(pl.wallet, 100); pl.inBattle = true; const cpuBeast = BEAST_KEYS[0]; const bId = `bgauntlet${uid()}`; battles.set(bId, { p1id: CPU_ID, p2id: id, st1: getStartState(cpuBeast), st2: getStartState(pl.beast), turnId: CPU_ID, logs: [{t: `¡Torre de Batalla!`, c: 'hi'}], isCpu: true, isGauntlet: true, gauntletIndex: 0, cpuIsP1: true, cpuBeast }); send(ws, { type: 'battle_start', battleId: bId, role: 'p2', opponent: 'Zodiac Master', opponentBeast: cpuBeast, isCpu: true, isGauntlet: true }); await pushLobby(); setTimeout(() => { pushCpuBattle(bId); scheduleGauntletCpuTurn(bId); }, 200); }
  ...
  ws.on('close', async () => {
    try {
      ...
      for (const [bId, b] of battles) { if (b.isTraining && (b.p1id === id || b.p2id === id)) { battles.delete(bId); } else if (b.isTeamBattle && (b.p1id === id || b.p2id === id)) { const otherId = b.p1id === id ? b.p2id : b.p1id; const winnerTeam = b.p1id === otherId ? b.team1 : b.team2; const winnerRemainingHp = winnerTeam.reduce((sum, st) => sum + Math.max(0, st.hp), 0); await endTeamBattle(bId, otherId, id, winnerRemainingHp); } else if (b.isGauntlet && b.p2id === id) { await endGauntlet(bId, id, false); } else if (b.isCpu && b.p2id === id) { battles.delete(bId); } else if (b.p1id === id || b.p2id === id) { const otherId = b.p1id === id ? b.p2id : b.p1id; await endBattle(bId, otherId, id, 0, true); } }
