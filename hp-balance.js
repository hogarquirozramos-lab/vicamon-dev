...
async function markTxProcessed(signature) {
...
async function addHP(wallet, hp) {
  await pool.query(`INSERT INTO players (wallet, hp, locked_hp) VALUES ($1, $2, 0) ON CONFLICT (wallet) DO UPDATE SET hp = players.hp + $2`, [wallet, hp]);
  return await getHP(wallet);
}
...
async function lockHP(wallet, amount = 100) {
  ...
  try {
    ...
    if (currentHp < amount) { await client.query('ROLLBACK'); return false; }
...
async function settleMatch(winnerWallet, loserWallet, winnerHp) {
  ...
  try {
    ...
    return { winnerNewHp, platformHp, platformUsdc: parseFloat((platformHp * USDC_PER_HP).toFixed(3)) };
...
async function cashout(wallet) {
  ...
  try {
    ...
    if (hp <= 0) { await client.query('ROLLBACK'); return { ok: false, reason: 'no_hp', hp: 0, usdc: 0 }; }
  ...
  } catch(e) { await client.query('ROLLBACK'); return { ok: false, reason: 'db_error', hp: 0, usdc: 0 }; } finally { client.release(); }
}
