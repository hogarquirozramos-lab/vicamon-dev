...
async function connectPhantom() {
  await new Promise(r => setTimeout(r, 100)); const phantom = getPhantom();
  if (!phantom || !phantom.isPhantom) {
    ...
    document.getElementById('no-phantom').style.display='block'; document.getElementById('no-phantom').innerHTML = 'Phantom no detectado. <a href="https://phantom.app" target="_blank" style="color:#F0997B;text-decoration:underline">Instalalo aqui</a> y recarga la pagina.'; document.getElementById('btn-phantom').style.display='none'; return;
  }
  try { const resp = await phantom.connect(); myWallet = resp.publicKey.toString(); document.getElementById('btn-phantom').style.display='none'; document.getElementById('wallet-connected').style.display='block'; document.getElementById('wallet-addr').textContent = myWallet.slice(0,8)+'...'+myWallet.slice(-6); document.getElementById('wallet-hp').textContent = 'Verificando...'; const sn = document.getElementById('step-name'); if(sn){ sn.style.opacity='1'; sn.style.pointerEvents='auto'; } await checkHPNow(true); } catch(e) { console.error('Phantom error:', e); alert('No se pudo conectar Phantom.'); }
}
...
async function checkHPNow(fromConnect=false) { if (!myWallet) return; try { const res = await fetch('/hp?wallet='+myWallet); const data = await res.json(); const hp = data.hp || 0; const loginHp = document.getElementById('wallet-hp'); if(loginHp){ loginHp.textContent=hp+' HP'; loginHp.style.color=hp>=100?'#5DCAA5':'#EF9F27'; } updateHPDisplay(hp); if (data.stats) updateProfileUI(data.stats); if (hp >= 100) { document.getElementById('step-charge').style.display='none'; document.getElementById('step-name').style.opacity='1'; document.getElementById('step-name').style.pointerEvents='auto'; } else { document.getElementById('step-charge').style.display='block'; document.getElementById('step-name').style.opacity='1'; document.getElementById('step-name').style.pointerEvents='auto'; } } catch(e) { document.getElementById('wallet-hp').textContent = 'Error al verificar'; } }
...
function show(id){ document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active')); document.getElementById(id).classList.add('active'); if(id === 's-login' || id === 's-lobby' || id === 's-profile' || id === 's-team-select') playMusic('lobby'); if(id === 's-battle' || id === 's-result') playMusic('batalla'); }
...
function buildTeamPickGrid() {
  ...
  let html='';
  keys.forEach(([k,b])=>{
    html+=`<div class="bcard" id="tpc-${k}" onclick="toggleTeamBeast('${k}')">
...
function enterLobby(){ if(ws && ws.readyState === 1) { show('s-lobby'); ws.send(JSON.stringify({type:'ping'})); } else { if(!myBeast) myBeast = 'aries'; // Temporal para no romper el servidor connectWS(); } }
...
function challengeGauntlet() { if(!ws || ws.readyState !== 1) return alert('Espera, estás conectando al servidor.'); if(!confirm('¿Iniciar la Torre de Batalla? Apostarás 100 HP. Si derrotas a los 12 Vicamons, ganarás 100 HP extra. Si caes, perderás tus 100 HP.')) return; ws.send(JSON.stringify({type:'challenge_gauntlet'})); }
...
function surrender() { if(!confirm('¿Estás seguro de rendirte? Si es una batalla de HP o Torre, perderás tu apuesta.')) return; if(ws && ws.readyState === 1) ws.send(JSON.stringify({type:'surrender', battleId})); }
...
function handleMsg(m){
  ...
  if(m.type==='gauntlet_next'){ gauntletBattleId = m.battleId; gauntletSelectedBeast = myBeast; const b = BEASTS[m.nextBeast]; document.getElementById('g-title').textContent = `¡Jefe ${m.round - 1}/12 derrotado!`; document.getElementById('g-sub').innerHTML = `Tu HP se ha restaurado a 100.<br>El próximo rival es <strong style="color:#CFA9EC">${b.name}</strong> (${m.round}/12).<br>¿Quieres cambiar de Vicamon?`; const picker = document.getElementById('g-beast-picker'); picker.innerHTML = Object.entries(BEASTS).map(([k,b])=>`<div class="bcard" id="gbc-${k}" style="padding:5px" onclick="selectGauntletBeast('${k}')"><img src="${b.img}" alt="${b.name}" style="width:50px;height:50px"><div class="bname" style="font-size:10px">${b.name}</div></div>`).join(''); document.getElementById('gbc-'+myBeast)?.classList.add('sel'); document.getElementById('modal-gauntlet').classList.remove('hidden'); return; }
  if(m.type==='challenged'){ pendingFrom=m.fromId; pendingIsTraining = !!m.isTraining; const b=BEASTS[m.fromBeast]||{name:m.fromBeast,img:''}; document.getElementById('ch-img').src=b.img; document.getElementById('ch-title').textContent=`¡Reto de ${m.fromName}!`; document.getElementById('ch-sub').textContent=pendingIsTraining ? `${m.fromName} quiere un ENTRENAMIENTO con su ${b.name}. (Sin apostar HP)` : `${m.fromName} quiere batallar con su ${b.name}. ¿Aceptas el combate?`; document.getElementById('modal-challenged').classList.remove('hidden'); startChallengeBeep(); }
  if(m.type==='battle_start'){ battleId=m.battleId; myRole=m.role; oppName=m.opponent; oppBeast=m.opponentBeast; const empty={hp:100,maxHp:100,poisonDmg:0,poisonTurns:0,burnDmg:0,burnTurns:0,shield:0,shieldReflect:0,reflect50:0,stun:false,recharge:0,regen:0,regenTurns:0,blind:0,weakAtk:0,weaken:0,corrode:0,analyzed:0,lastDmgReceived:0,pp:[]}; mySt={...empty}; oppSt={...empty}; window._isCpuBattle=!!m.isCpu; window._isTrainingBattle=!!m.isTraining; window._isGauntlet=!!m.isGauntlet; const isCpu=!!m.isCpu; const isTraining=!!m.isTraining; let startMsg=''; if(isTraining) startMsg = `¡Entrenamiento amistoso! ${myName} (${BEASTS[myBeast]?.name}) vs ${oppName} (${BEASTS[oppBeast]?.name})`; else if(isCpu) startMsg = `¡Zodiac Master invoca a ${BEASTS[oppBeast]?.name||oppBeast}! Prepárate...`; else startMsg = `¡Combate! ${myName} (${BEASTS[myBeast]?.name}) vs ${oppName} (${BEASTS[oppBeast]?.name})`; show('s-battle'); renderBattle(!isCpu,[{t:startMsg,c:'hi'}]); }
  if(m.type==='battle_state'){ const me=myRole==='p1'?m.p1:m.p2; const opp=myRole==='p1'?m.p2:m.p1; myBeast = me.beast || myBeast; oppBeast = opp.beast || oppBeast; const prevMyHp=mySt.hp, prevOppHp=oppSt.hp; mySt=me.state; oppSt=opp.state; if(mySt.hp<prevMyHp) animHit('me',prevMyHp-mySt.hp); if(oppSt.hp<prevOppHp) animHit('opp',prevOppHp-oppSt.hp); renderBattle(m.yourTurn,m.logs); }
  if(m.type==='hp_updated'){ updateHPDisplay(m.hp); myCurrentHP=m.hp; }
  if(m.type==='cashout_result'){ const btn=document.getElementById('btn-cashout'); if(!m.ok){ if(btn){btn.disabled=false;btn.textContent='💰 Cashout';} alert('Error en cashout: '+m.reason); return; } if(m.status==='confirmed'){ if(btn){btn.disabled=false;btn.textContent='💰 Cashout';} updateHPDisplay(0); alert(`✓ Cashout exitoso\n${m.hp} HP = ${m.usdc} USDC enviados a tu wallet`); } }
...
function renderLobby(others){ _lastLobbyPlayers=others; const list=document.getElementById('players-list'); const myHp=myCurrentHP; const hpWarnEl=document.getElementById('lobby-hp-warn'); if(hpWarnEl) hpWarnEl.style.display=myHp<100?'block':'none'; if(!others.length){list.innerHTML='<p class="empty-lobby">No hay otros jugadores en el lobby...<br>Comparte la URL con tus amigos</p>';return;} list.innerHTML=others.map(p=>{ const b=BEASTS[p.beast]||{name:p.beast,img:''}; const rivalHp=p.hp||0; const can1v1=myHp>=100&&rivalHp>=100; const can3v3=myHp>=300&&rivalHp>=300; const hpColor=rivalHp>=100?'#5DCAA5':'#F0997B'; return `<div class="p-row"><div class="p-info"><img class="p-img" src="${b.img}" alt="${b.name}"><div><div class="p-name">${p.name}</div><div class="p-beast">${b.name} · <span style="color:${hpColor};font-size:10px">${rivalHp} HP</span></div></div></div><div style="display:flex;gap:6px"><button class="btn btn-sm" style="background:rgba(130,80,180,.15);border-color:rgba(130,80,180,.35);color:#CFA9EC" onclick="openChallengeMenu(${p.id},'${p.name}', true)">⚔️ Retar</button></div></div>`; }).join(''); }
async function doCashout(){ const btn=document.getElementById('btn-cashout'); if(btn){btn.disabled=true;btn.textContent='Procesando...';} if(!ws || ws.readyState !== 1){ if(btn){btn.disabled=false;btn.textContent='💰 Cashout';} alert('Debes entrar al lobby primero para conectar con el servidor.'); return; } ws.send(JSON.stringify({type:'cashout'})); }
function updateHPDisplay(hp){ myCurrentHP = hp || 0; const el=document.getElementById('pick-hp-val'); if(el){ el.textContent=hp+' HP'; el.style.color=hp>=100?'#5DCAA5':'#EF9F27'; } const elUsdc=document.getElementById('pick-usdc-val'); if(elUsdc) elUsdc.textContent='= '+(hp*0.001).toFixed(3)+' USDC'; const loginHp=document.getElementById('wallet-hp'); if(loginHp){ loginHp.textContent=hp+' HP'; loginHp.style.color=hp>=100?'#5DCAA5':'#EF9F27'; } const profHp=document.getElementById('profile-hp'); if(profHp){ profHp.textContent=hp+' HP'; profHp.style.color=hp>=100?'#5DCAA5':'#EF9F27'; } const profUsdc=document.getElementById('profile-usdc'); if(profUsdc) profUsdc.textContent='= '+(hp*0.001).toFixed(3)+' USDC'; const btn=document.getElementById('btn-cashout'); if(btn){ btn.style.display=hp>0?'inline-block':'none'; btn.disabled=false; btn.textContent='💰 Cashout'; } const warn=document.getElementById('low-hp-warning'); if(warn) warn.style.display=hp<100?'block':'none'; const charge=document.getElementById('step-charge'); if(charge) charge.style.display = hp<100 ? 'block' : 'none'; const depHtml = hp<100 ? depositWidgetHTML() : ''; const pickDep = document.getElementById('pick-deposit-widget'); if(pickDep) pickDep.innerHTML = depHtml; const lobbyDep = document.getElementById('lobby-deposit-widget'); if(lobbyDep) lobbyDep.innerHTML = depHtml; const profDep = document.getElementById('profile-deposit-widget'); if(profDep) profDep.innerHTML = depHtml; if(document.getElementById('s-lobby')?.classList.contains('active')){ const cur = document.getElementById('players-list'); if(cur) renderLobbyFromCache(); updateLobbyBadge(); } }

// MODIFICADO: Abre el menú de modos
...
function acceptChallenge(){ document.getElementById('modal-challenged').classList.add('hidden'); stopChallengeBeep(); if(pendingFrom!==null) ws.send(JSON.stringify({type:'accept',fromId:pendingFrom, isTraining: pendingIsTraining})); pendingIsTraining=false; pendingFrom=null; }
function rejectChallenge(){ document.getElementById('modal-challenged').classList.add('hidden'); stopChallengeBeep(); pendingFrom=null; pendingIsTraining=false; }
...
function handleChatMessage(m){ const chatBox = document.getElementById('chat-box'); if(chatBox.querySelector('.chat-empty')) chatBox.innerHTML = ''; const msgDiv = document.createElement('div'); msgDiv.className = 'chat-msg'; msgDiv.innerHTML = `<span class="chat-name">${escapeHtml(m.name)}:</span> <span style="color:rgba(255,255,255,.8)">${escapeHtml(m.text)}</span>`; chatBox.appendChild(msgDiv); while(chatBox.children.length > 50) { chatBox.removeChild(chatBox.firstChild); } chatBox.scrollTop = chatBox.scrollHeight; }
function renderLeaderboard(top) { const podium = document.getElementById('leaderboard-podium'); if (!podium) return; if (!top || top.length === 0) { podium.innerHTML = '<div style="flex:1;color:rgba(255,255,255,.3);text-align:center;font-size:11px;padding:20px 0">Gana batallas reales para aparecer aquí</div>'; return; } podium.innerHTML = top.map((p, i) => { const medals = ['🥇', '🥈', '🥉']; const colors = ['#F5A623', '#C0C0C0', '#CD7F32']; const name = p.last_name || 'Anónimo'; const wins = p.wins || 0; const losses = p.losses || 0; return `<div style="flex:1;background:rgba(255,255,255,.04);border:0.5px solid ${colors[i]};border-radius:10px;padding:10px 6px;text-align:center;backdrop-filter:blur(4px)"><div style="font-size:20px;margin-bottom:2px">${medals[i] || '🏆'}</div><div style="font-size:12px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${name}</div><div style="font-size:9px;color:rgba(255,255,255,.5);margin-top:3px">${wins}V · ${losses}D</div></div>`; }).join(''); }
function renderBattle(yourTurn, logs){ document.getElementById('f-me').innerHTML=panelHTML(mySt,myBeast,myName+' (tú)','me'); document.getElementById('f-opp').innerHTML=panelHTML(oppSt,oppBeast,oppName,'opp'); const orb=document.getElementById('turn-orb'); if(orb) orb.style.display=yourTurn?'block':'none'; const locked=!yourTurn||mySt.stun||mySt.recharge>0; let tb=''; if(yourTurn){ if(mySt.stun) tb='Estás aturdido — pierdes este turno'; else if(mySt.recharge>0) tb=`⚡ Recargando — espera ${mySt.recharge} turno(s)`; else tb='<span>Tu turno</span> — elige un ataque'; } else tb='Turno del rival...'; document.getElementById('turn-bar').innerHTML=tb; const b=BEASTS[myBeast]; document.getElementById('atk-grid').innerHTML=b.attacks.map((a,i)=>{ const tags=[]; if(a.pierce) tags.push('<span class="atk-tag tag-pierce">Ignora escudo</span>'); if(a.fx==='double') tags.push('<span class="atk-tag tag-nobreak">Doble golpe</span>'); if(a.fx==='triple') tags.push('<span class="atk-tag tag-nobreak">Triple golpe</span>'); if(a.risk||a.self>0) tags.push(`<span class="atk-tag tag-risk">Riesgo${a.self>0?' -'+a.self+' HP':''}</span>`); if(a.buff) tags.push('<span class="atk-tag tag-buff">Buff</span>'); if(a.dot) tags.push('<span class="atk-tag tag-dot">Daño/turno</span>'); if(a.debuff) tags.push('<span class="atk-tag tag-debuff">Debuff</span>'); const currentPp = mySt.pp ? mySt.pp[i] : undefined; const maxPp = a.pp === undefined ? 99 : a.pp; const ppLeft = currentPp === undefined ? maxPp : currentPp; const isDisabled = locked || ppLeft <= 0; const ppText = maxPp === 99 ? '∞' : `${ppLeft}/${maxPp}`; return `<button class="atk-btn" ${isDisabled?'disabled':''} onclick="doAttack(${i})"><div class="atk-top"><div class="atk-name">${a.n}</div><div class="atk-dmg ${dmgClass(a)}">${dmgLabel(a)}</div></div><div class="atk-tags">${tags.join('')}</div><div class="atk-desc">${a.desc}</div><div style="display:flex;justify-content:space-between;align-items:center"><div class="atk-acc">${a.acc}% precisión</div><div class="atk-acc" style="color:rgba(255,255,255,.5)">PP: ${ppText}</div></div></button>`; }).join(''); if(logs&&logs.length){ const lb=document.getElementById('log-box'); lb.innerHTML=logs.map(l=>`<div class="ll lc-${l.c||'normal'}">${l.t}</div>`).join(''); lb.scrollTop=lb.scrollHeight; } }
function doAttack(i){ animAttack('me'); try { const atk = BEASTS[myBeast].attacks[i]; if(atk.d === 0) playSfx('curacion'); else playSfx('ataque'); } catch(e) { console.error("Audio error:", e); } ws.send(JSON.stringify({type:'attack',battleId,index:i})); }
