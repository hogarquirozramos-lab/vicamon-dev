...
function handleMsg(m){
  ...
  if(m.type === 'team_force_switch'){ alert(m.reason); openSwitchMenu(); }
...
function acceptChallenge(){
  ...
  if(pendingIs3v3) { teamSelectionMode = '3v3'; selectedTeam = []; document.getElementById('ts-mode-title').textContent = 'Entrenamiento 3 vs 3 (Elige 3)'; buildTeamPickGrid(); show('s-team-select'); }
  else { ws.send(JSON.stringify({type:'accept', fromId:pendingFrom, isTraining: pendingIsTraining})); pendingIsTraining=false; pendingFrom=null; }
}
