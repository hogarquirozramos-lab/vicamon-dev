function show(id){ document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active')); document.getElementById(id).classList.add('active'); if(id === 's-login' || id === 's-pick' || id === 's-lobby' || id === 's-profile' || id === 's-team-select' || id === 's-lab' || id === 's-tournament' || id === 's-starter') playMusic('lobby'); if(id === 's-battle' || id === 's-result') playMusic('batalla'); }
